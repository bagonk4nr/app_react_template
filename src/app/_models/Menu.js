...
class Menu extends React.Component {
    ...
    htmlMenu = () => {
     ...
     if (this.dataMenu[k].idsubmenu != null ) {
       ...
       submenu1.push(<Link to={{pathname: "/", state: {url: this.dataMenu[k].links }}} className="dropdown-item" key={7 * (k+6)} >{ this.dataMenu[k].submenu } </Link> );


     if (this.dataMenu[k].idsubmenu != null && captions1 !== this.dataMenu[k].captions) {
       submenu.push(<Dropdown.Menu className="dropdown-menu" key={6 * (k+d)} > { submenu1 } </Dropdown.Menu>);
     }
     ...
     if (this.dataMenu[k].idsubmenu != null && captions !== this.dataMenu[k].captions) {
       // console.log({submenu});
       btnx.push(<Dropdown key={5 * (k+d)}><Dropdown.Toggle key={8 * (k+d)} variant="info" id="dropdown-basic"> { this.dataMenu[k].captions } { submenu } </Dropdown.Toggle></Dropdown>);
       btnx.push(<span key={2 * (k+s)}>&emsp;</span>);
     } else if (this.dataMenu[k].idsubmenu == null) {
       btnx.push(<button type="button" key={3 * (k+5)} className="btn btn-secondary" id={ this.dataMenu[k].links } >{ this.dataMenu[k].captions } </button> );
     ...
     captions = this.dataMenu[k].captions;
       if (this.checkIsInt.isInt(f / this.jmlhButton) === 1) {
           // console.log({ btnx });
           getHtml.push(<div  key={1 * (f + 3)} className="card rbodycorners"><div className="card-body" key={3 * (f + 5)} > <div className="btn-group" key={5* (f + 7)}> { btnx } </div></div></div>);
           ...
           // console.log({ btnx });
           getHtml.push(<div key={2 * (f + 4)} className="card rbodycorners"><div className="card-body" key={4* (f + 6)} > <div className="btn-group" key={6* (f + 9)} > { btnx } </div></div></div>);
