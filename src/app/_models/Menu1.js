                        ...
                        if (f <= this.newArr[a]) {
                            ...
                            if (this.dataMenu[k].idsubmenu != null) {
                                ...
                                if (this.dataMenu[k].idsubmenu != null && captions1 !== this.dataMenu[k].captions) {
                                    submenu.push( < div className = "dropdown-menu"
                                ...
                                if (this.dataMenu[k].idsubmenu != null && captions !== this.dataMenu[k].captions) {
                                    // console.log({submenu});
                                    btnx.push( < div className = "dropdown"
                                        key = { 5 * (k + d) } > < button className = "btn btnmenu dropdown-toggle"
                                        type = "button"
                                        ...
                                        aria-expanded = "false"
                                        key = { 8 * (k + d) } > <
                                        i className = { this.dataMenu[k].icon + " iconchangers" } > < /i> <div className="lineheight"> </div > { this.dataMenu[k].captions } < /button >  { submenu } < /div > );
                                    btnx.push( < span key = { 2 * (k + s) }
                                        ...
                                        if (this.checkIsInt.isInt(f / this.jmlhButton) === 1) {
                                            ...
                                            getHtml.push( <div className="card-deck"> < div key = { 1 * (f + 3) }
                                                ...
                                                // console.log({ btnx });
                                                getHtml.push( <div className="row"> < div key = { 2 * (f + 4) }
                                                    className = "card rbodycorners w-100 p-0 w-0" > < div className = "card-body"
                                                    key = { 4 * (f + 6) } > < div className = "btn-group"
                                                    key = { 6 * (f + 9) } > { btnx } < /div></div >< /div>< /div>);
